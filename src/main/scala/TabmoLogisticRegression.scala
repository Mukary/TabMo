...
object TabmoLogisticRegression{
  ...
  def main(args: Array[String]){
    ...
    val labelIndex = rawData.columns.indexOf("label")
    ...
    val data = rawData.rdd.map(row => {
       ...
       LabeledPoint(bool2double(row.getBoolean(labelIndex)), Vectors.dense(doubleArray.map(e => {
         if(e.isInstanceOf[Boolean])
           bool2double(e.asInstanceOf[Boolean])
         else
          e.asInstanceOf[Double]
       })))
     })
